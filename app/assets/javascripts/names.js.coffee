...
replot = (results) ->
  ...
  y2.domain y.domain()
  focus.selectAll(".area").data(results).attr("id", (d) -> d[0].orbital )
    .attr("class", insertOrbitals())
    .attr "d", area
  focus.call(yAxis).append("g").attr("class", "y axis").call yAxis
  context.selectAll(".area").data(results).attr("id", (d) -> d[0].orbital )
    .attr("class", insertOrbitals())
    .attr "d", area2
  return
...
dosPlot = (results) ->
  ...
  x2.domain x.domain()
  y2.domain y.domain()
  focus.selectAll(".area").data(results).enter().append("path").attr("id", (d) -> d[0].orbital )
    .attr("class", insertOrbitals())
    .attr "d", area
  focus.append("g").attr("class", "x axis").attr("transform", "translate(0," + height + ")").call xAxis
  focus.append("g").attr("class", "y axis").call yAxis
  context.selectAll(".area").data(results).enter().append("path").attr("id", (d) -> d[0].orbital )
    .attr("class", insertOrbitals() )
    .attr "d", area2
...
svg = d3.select("#chart").append("svg:svg").attr("width", width + margin.left + margin.right)
  .attr("height", height + margin.top + margin.bottom)

area = d3.svg.area().interpolate("monotone").x((d) -> x d.x )
  .y0(height).y1((d) -> y d.y )
area2 = d3.svg.area().interpolate("monotone").x((d) -> x2 d.x )
  .y0(height2).y1((d) -> y2 d.y )
